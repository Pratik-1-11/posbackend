...
-- Migration: 100_fifo_accounting_logic.sql
-- Purpose: Implement automatic COGS calculation and FIFO inventory accounting.

-- 1. Create a function to calculate COGS for a sale and post to Ledger
CREATE OR REPLACE FUNCTION public.post_cogs_for_sale(
    p_sale_id UUID,
    p_tenant_id UUID,
    p_branch_id UUID,
    p_cashier_id UUID
...
BEGIN
    -- Resolve accounts
    SELECT id INTO v_acct_inventory FROM public.accounts WHERE tenant_id = p_tenant_id AND system_code = 'INVENTORY' LIMIT 1;
    SELECT id INTO v_acct_cogs FROM public.accounts WHERE tenant_id = p_tenant_id AND system_code = 'COGS' LIMIT 1;

    IF v_acct_inventory IS NULL OR v_acct_cogs IS NULL THEN
        RETURN 0;
    END IF;
    ...
    -- Loop through sale items to calculate COGS based on batch cost
    FOR v_item IN (
        SELECT 
            si.quantity,
            COALESCE(pb.cost_price, p.cost_price, 0) as unit_cost
...
-- 2. Enhance process_pos_sale to call COGS logic
...
BEGIN
  -- [Existing Shift, Idempotency, and Invoice logic remains the same]
  ...
  -- 9. NEW: COGS AUTOMATION (FIFO-Aware)
