...
-- Robustness: Self-healing check for table existence to prevent "relation does not exist" errors.
...
BEGIN
    ...
    ELSE
        ...
        -- Add branch_id if missing (shouldn't happen but defensive)
    ...
    -- Drop and Recreate PK to ensure it's (tenant_id, branch_id, fiscal_year)
    ALTER TABLE public.invoice_counters DROP CONSTRAINT IF EXISTS invoice_counters_pkey;
