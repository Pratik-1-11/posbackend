...
-- Migration: 108_fix_process_pos_sale_overloading.sql
-- Purpose: Resolve function overloading ambiguity for process_pos_sale RPC

-- 1. Drop all existing overloaded versions of process_pos_sale
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        WHERE n.nspname = 'public' AND p.proname = 'process_pos_sale'
    ) LOOP
        EXECUTE 'DROP FUNCTION ' || r.nspname || '.' || r.proname || '(' || r.args || ')';
    END LOOP;
END $$;

-- 2. Recreate the latest version (from 107)
