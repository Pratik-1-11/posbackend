...
BEGIN
  ...
  LOOP
    ...
    -- Throw error if stock becomes negative (optional, but good for business)
    -- IF (SELECT stock_quantity FROM public.products WHERE id = (v_item->>'product_id')::UUID) < 0 THEN
  ...
  IF v_credit_amount > 0 THEN
    ...
    -- Use the existing add_customer_transaction function
  ...
  -- Assuming audit_logs table exists, if not, skip
  -- INSERT INTO public.audit_logs (user_id, action, target_type, target_id, details)
  -- VALUES (p_cashier_id, 'SALE_CREATED', 'sales', v_sale_id, jsonb_build_object('invoice', v_invoice_number, 'total', p_total_amount));
