-- Migration: 041_auto_je_for_sales.sql
-- Purpose: Auto-generate double-entry journals for POS sales in process_pos_sale RPC.
...
BEGIN
  ...
  -- If total is 120, discount 10, VAT 13% derived. 
  -- We trust the frontend parameters for the basic arithmetic in this RPC but will log them properly below.
  ...
  -- 6. Credit Logic (If credit part exists)
  v_credit_amount := 0;
  IF (p_payment_method = 'credit' OR (p_payment_method = 'mixed' AND (p_payment_details->>'credit')::NUMERIC > 0)) AND p_customer_id IS NOT NULL THEN
    v_credit_amount := COALESCE((p_payment_details->>'credit')::NUMERIC, p_total_amount);
  ...
  -- 7. DOUBLE-ENTRY ACCOUNTING HOOKS
  -- Lookup Accounts for the tenant
  ...
  -- Check if chart of accounts is fully mapped (if not, we gracefully skip to not break the system)
  IF v_acct_cash IS NOT NULL AND v_acct_sales IS NOT NULL THEN
    ...
    -- Debit Cash/Bank (Total Amount Paid)
    ...
    -- Debit Accounts Receivable (Credit Amount)
    ...
    -- E.g. Subtotal 100, Discount 10 => Taxable 90 => VAT 11.7. Total = 101.7
    -- Debits: Cash 101.7, Discount 10. (Total Debit 111.7)
    -- Credits: Revenue 100, VAT 11.7. (Total Credit 111.7)
    -- If p_taxable_amount = 90, p_sub_total should be 100.
