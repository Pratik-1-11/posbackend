...
BEGIN
  ...
  SELECT COUNT(*) INTO v_profiles FROM public.profiles WHERE tenant_id IS NOT NULL;
  SELECT COUNT(*) INTO v_products FROM public.products WHERE tenant_id IS NOT NULL;
  ...
  RAISE NOTICE 'Profiles with tenant: %', v_profiles;
  RAISE NOTICE 'Products with tenant: %', v_products;
  RAISE NOTICE '';
  RAISE NOTICE 'Next: Verify your data in Supabase Table Editor';
  RAISE NOTICE '';
END $$;
